...
const SongsTableView = (props) => {
    ...
    const iconMaker = (permissionId, model) => {
        if (permissionId == 1 && model.isactive == 1) return <RemoveRedEye key={permissionId} color="primary" className="ml-2 pointer" onClick={() => { handleOpen(); setSelected({ ...model }) }} />
        else if (permissionId == 2 && model.isactive == 1) return <Edit key={permissionId} color="action" className="ml-2 pointer" onClick={() => {
            fetchById(model.id)
        }} />
        else if (permissionId == 3 && model.isactive == 1) return <DeleteForeverSharp key={permissionId} color="error" className="ml-2 pointer" onClick={
            () => {
                setSelected({ ...model })
                setDeleting(true)
        ...
        else if (permissionId == 5 && model.isactive == 1) return <Flag key={permissionId} color="error" className="ml-2 pointer" onClick={
            () => {
                fetch(`deactivate/song/${model.id}`,
                    {
                        method: 'PUT', // or 'PUT'
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then(result => result.json())
                    .then(data => {
                        showAlert("Se ha desactivado con exito!", "success")
                        setRows(_.sortBy(data, ['id']))
                    }).catch(error => {
                        showAlert(props.errorMessage, "error")
                    })
        ...
        else if (permissionId == 5 && model.isactive == 0) return <Flag key={permissionId} color="success" className="ml-2 pointer" onClick={
            () => {
                fetch(`activate/song/${model.id}`,
                    {
                        method: 'PUT', // or 'PUT'
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then(result => result.json())
                    .then(data => {
                        showAlert("Se ha activado con exito!", "success")
                        setRows(_.sortBy(data, ['id']))
                    }).catch(error => {
                        showAlert(props.errorMessage, "error")
                    })
    ...
    const handleCloseDelete = () => (setDeleting(false))
        ...
        <div className="row">
            ...
            }
            {
                ...
                (mounted ?
                    ...
                    >
                        <Fade in={deleting}>
                            <div className={classes.paperDelete}>
                                <h2 id="transition-modal-title white mb-3">¿Esta seguro que desea borrar la cancion?</h2>
                                <h3 id="transition-modal-title white mb-3">Al borrarla se quitaran tambien las canciones de las playlist</h3>
                                <div className="row">
                                    ...
                                    <div className="p-2 col-6">
                                        <button
                                            type="button"
                                            className={'btn-error w-100 mt-3 mb-2'}
                                            onClick={() => {
                                                handleCloseDelete()
                                            }}
                                        >
